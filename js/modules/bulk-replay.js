...
export function setupBulkReplay() {
    ...
    const closeModalBtn = document.querySelector('#bulk-config-modal .close-modal');
    ...
    function checkPayloadMarkers() {
        ...
        bulkReplayBtn.disabled = !hasMarkers;
        bulkReplayBtn.classList.toggle('ready', hasMarkers);
    }
    ...
    if (bulkReplayBtn) {
        bulkReplayBtn.addEventListener('click', () => {
            ...
            document.getElementById('payload-count').textContent = matches.length;
    ...
    function populatePositionsContainer(matches) {
        ...
        matches.forEach((match, index) => {
            const cleanValue = escapeHtml(match.replace(/§/g, ''));
            ...
            card.innerHTML = `
                ...
                <div class="payload-options-numbers" style="display: none;">
                    <div class="form-row">
                        <div class="form-group"><label>From</label><input type="number" class="num-from-input form-control" data-index="${index}" value="1"></div>
                        <div class="form-group"><label>To</label><input type="number" class="num-to-input form-control" data-index="${index}" value="10"></div>
                        <div class="form-group"><label>Step</label><input type="number" class="num-step-input form-control" data-index="${index}" value="1"></div>
                    </div>
                </div>
            `;
            container.appendChild(card);

            card.querySelector('.payload-type-select').addEventListener('change', (e) => {
                ...
                simpleList.style.display = e.target.value === 'simple-list' ? 'block' : 'none';
                numbers.style.display = e.target.value === 'numbers' ? 'block' : 'none';
            });
        });
    }
    ...
    function updateAttackTypeUI(attackType) {
        ...
        positionsContainer.style.display = attackType === 'battering-ram' ? 'none' : 'block';
        batteringRamConfig.style.display = attackType === 'battering-ram' ? 'block' : 'none';
    }

    if (closeModalBtn) {
        closeModalBtn.addEventListener('click', () => bulkConfigModal.style.display = 'none');
    }

    window.addEventListener('click', (e) => {
        if (e.target === bulkConfigModal) bulkConfigModal.style.display = 'none';
    });
    ...
    if (startAttackBtn) {
        startAttackBtn.addEventListener('click', startBulkReplay);
    }

    if (bulkStopBtn) {
        bulkStopBtn.addEventListener('click', () => {
            const isPaused = bulkStopBtn.dataset.state === 'paused';
            state.shouldPauseBulk = !isPaused;
            bulkStopBtn.dataset.state = isPaused ? 'running' : 'paused';
            bulkStopBtn.title = isPaused ? 'Pause Attack' : 'Resume Attack';
            bulkStopBtn.innerHTML = isPaused ?
    ...
    if (verticalResizeHandle) {
        ...
        verticalResizeHandle.addEventListener('mousedown', () => isVerticalResizing = true);
    ...
    if (markPayloadItem) {
        markPayloadItem.addEventListener('mousedown', (e) => {
            e.preventDefault(); // Prevent editor from losing focus

            let range;
            const selection = window.getSelection();

            if (selection.rangeCount > 0 && elements.rawRequestInput.contains(selection.anchorNode)) {
            ...
            if (!range) {
                // As a fallback, insert at the beginning
                elements.rawRequestInput.focus();
                const newRange = document.createRange();
                newRange.setStart(elements.rawRequestInput, 0);
                range = newRange;
            }
            ...
            elements.rawRequestInput.dispatchEvent(event);

            contextMenu.classList.remove('show');
            savedRange = null; // Clear saved range after use
        });
    }
    ...
    async function startBulkReplay() {
        ...
        if (state.currentAttackType === 'battering-ram') {
            ...
            const sharedConfig = {
                ...
                numbers: type === 'numbers' ? {
                    from: parseInt(container.querySelector('.num-from-input').value, 10),
                    to: parseInt(container.querySelector('.num-to-input').value, 10),
                    step: parseInt(container.querySelector('.num-step-input').value, 10)
                } : null
            };
            ...
            document.querySelectorAll('.position-card').forEach((card, index) => {
                ...
                state.positionConfigs[index].list = type === 'simple-list' ? card.querySelector('.payload-list-input').value : '';
                state.positionConfigs[index].numbers = type === 'numbers' ? {
                    from: parseInt(card.querySelector('.num-from-input').value, 10),
                    to: parseInt(card.querySelector('.num-to-input').value, 10),
                    step: parseInt(card.querySelector('.num-step-input').value, 10)
                } : null;
            });
        }
        ...
        try {
            ...
            alert(`Error generating requests: ${error.message}`);
            return;
        }

        if (attackRequests.length === 0) {
            alert('No requests to send. Check payload configuration.');
            return;
        }

        if (attackRequests.length > 1000 && !confirm(`Generate ${attackRequests.length} requests?`)) {
            return;
        }

        bulkConfigModal.style.display = 'none';
        if (elements.rawResponseDisplay.textContent.trim()) {
        ...
        if (bulkStopBtn) {
            ...
            bulkStopBtn.innerHTML = `<svg viewBox="0 0 24 24" width="16" height="16"><path d="M6 4h4v16H6V4zm8 0h4v16h-4V4z" fill="currentColor" /></svg>`;
        }
        ...
        for (let i = 0; i < total; i++) {
            ...
            const { requestContent, payloads } = attackRequests[i];

            const row = document.createElement('tr');
            row.dataset.index = i;
            row.innerHTML = `
                <td>${i + 1}</td>
                <td>${escapeHtml(payloads.join(', '))}</td>
            ...
            row.addEventListener('click', () => {
                ...
                elements.resStatus.className = `status-badge status-${String(result.status)[0]}xx`;
                ...
                if (result.error) {
                    ...
                    result.headers.forEach((val, key) => rawResponse += `${key}: ${val}\n`);
                    rawResponse += '\n';
                    ...
                    if (elements.showDiffCheckbox?.checked && state.regularRequestBaseline) {
            ...
            try {
                ...
                if (!method || !path) throw new Error('Invalid request line');
                ...
                for (let j = 1; j < lines.length; j++) {
                    if (!isBody && lines[j].trim() === '') {
                        isBody = true; continue;
                    }
                    if (isBody) {
                        body += lines[j] + (j < lines.length - 1 ? '\n' : '');
                    } else {
                        const [key, value] = lines[j].split(/:\s*/, 2);
                        if (key && value) {
                ...
                bulkResults[i] = {
                    requestContent, status: response.status, statusText: response.statusText,
                    headers: response.headers, responseBody, size: responseBody.length,
                    duration, error: null
                };

                row.querySelector('.status-cell').textContent = `${response.status} ${response.statusText}`;
                row.querySelector('.size-cell').textContent = formatBytes(responseBody.length);
                row.querySelector('.time-cell').textContent = duration;

            } catch (error) {
                bulkResults[i] = {
                    requestContent, status: 'Error', error: error.message,
