...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const panelPort = browser.runtime.connect({ name: "rep-panel" });

    // Optional: Log connection status
    panelPort.onDisconnect.addListener(() => {
        console.log("Panel disconnected from background script. UI may not update.");
        // Potentially disable features that rely on the background script
    });
    ...
    // Initialize Features
    initTheme();
    initMultiTabCapture(panelPort); // Pass the port to the multi-tab module
    initExtractorUI();
    setupBulkReplay();
